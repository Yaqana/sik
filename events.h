...
public:
    ...
    players() const { return std::vector<std::string>(); }
...
public:
    static std::shared_ptr<NewGame>
    New(char *buffer, size_t len, uint32_t event_no);

    NewGame(uint32_t maxx, uint32_t maxy,
...
public:
    static std::shared_ptr<PlayerEliminated>
    New(char *buffer, size_t len, uint32_t event_no);
